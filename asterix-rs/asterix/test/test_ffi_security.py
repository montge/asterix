...
class TestFFIBufferSafety(unittest.TestCase):
    ...
    def test_empty_data(self):
        """Test parsing empty data doesn't crash."""
        result = asterix.parse(b'')
...
class TestFFIEdgeCases(unittest.TestCase):
    ...
    def test_parse_with_offset_beyond_data(self):
        """Test parse_with_offset with offset beyond data length."""
        data = b'\x30\x00\x10' + b'\x00' * 13
        records, new_offset = asterix.parse_with_offset(data, offset=1000, blocks_count=1)
        self.assertIsInstance(records, list)
...
class TestFFIRobustness(unittest.TestCase):
    ...
    def test_huge_blocks_count(self):
        """Test parse_with_offset with unreasonably large blocks_count."""
        data = b'\x30\x00\x10' + b'\x00' * 13
        records, new_offset = asterix.parse_with_offset(data, offset=0, blocks_count=999999999)
        # Should handle gracefully without allocating 1GB
        self.assertIsInstance(records, list)
