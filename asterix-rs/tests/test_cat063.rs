    ...
    match result {
        Ok(records) => {
            if !records.is_empty() {
                ...
                println!("✓ Parsed CAT063 with data items: {} record(s)", records.len());
            }
        }
        Err(e) => {
...
fn test_cat063_api_usage() {
    ...
    match (result_verbose, result_quiet) {
        (Ok(records_v), Ok(records_q)) => {
            ...
            println!("✓ CAT063 API usage: verbose={}, quiet={} records", records_v.len(), records_q.len());
        }
        (Ok(records_v), Err(_)) => {
            println!("⚠ CAT063 verbose succeeded ({} records) but quiet failed", records_v.len());
        }
        (Err(_), Ok(records_q)) => {
            println!("⚠ CAT063 quiet succeeded ({} records) but verbose failed", records_q.len());
        }
        (Err(e1), Err(e2)) => {
    ...
    match result_filter {
        Ok(records) => {
            ...
            println!("✓ Category filter working: {} CAT063 records", records.len());
        }
        Err(e) => {
