"""
Python integration test for CAT015 parsing
INCS Target Reports

This test verifies the Python API for parsing ASTERIX Category 015 data.
...
Test Cases:
- TC-PY-CAT015-001: Parse CAT015 binary data
- TC-PY-CAT015-002: Verify parsed data structure
- TC-PY-CAT015-003: Test error handling
"""
...
class TestCAT015:
    """Test Category 015 (INCS Target Reports) parsing"""

    def test_parse_cat015_packet(self):
        """
        Test parsing CAT015 packet

        Requirement: REQ-HLR-001, REQ-LLR-015-010
        Verification: TC-PY-CAT015-001
        """
        # Create minimal valid CAT015 packet
        # Structure: [Category: 0x0F] [Length MSB: 0x00] [Length LSB: 0x03]
        cat015_packet = bytearray([
            0x0F,  # Category 15
            0x00,  # Length MSB
            0x03,  # Length LSB = 3 bytes (header only)
        ])

        result = asterix.parse(cat015_packet)
        ...
        if len(result) > 0:
            assert result[0]['category'] == 15, f"Should identify as CAT015"

    def test_parse_cat015_with_data_items(self):
        """
        Test parsing CAT015 with data items

        Requirement: REQ-LLR-015-010
        Verification: TC-PY-CAT015-002
        """
        # Create CAT015 packet with I015/010 (Data Source Identifier)
        # Structure: [Category: 0x0F] [Length MSB: 0x00] [Length LSB: 0x08] [FSPEC: 0x80] [I015/010: 2 bytes]
        cat015_packet = bytearray([
            0x0F,  # Category 15
            0x00,  # Length MSB
    ...
    def test_cat015_error_handling(self):
        """
    ...
    def test_cat015_api_usage(self):
        """
        Test Python API usage for CAT015

        Requirement: REQ-HLR-001
        Verification: TC-PY-CAT015-004
        """
        ...
        result_verbose = asterix.parse(cat015_packet, verbose=True)
        result_quiet = asterix.parse(cat015_packet, verbose=False)
