"""
Python integration test for CAT205 parsing
Service Status and Message

This test verifies the Python API for parsing ASTERIX Category 205 data.
...
Test Cases:
- TC-PY-CAT205-001: Parse CAT205 binary data
- TC-PY-CAT205-002: Verify parsed data structure
- TC-PY-CAT205-003: Test error handling
"""
...
class TestCAT205:
    """Test Category 205 (Service Status and Message) parsing"""

    def test_parse_cat205_packet(self):
        """
        Test parsing CAT205 packet

        Requirement: REQ-HLR-001, REQ-LLR-205-010
        Verification: TC-PY-CAT205-001
        """
        # Create minimal valid CAT205 packet
        # Structure: [Category: 0xCD] [Length MSB: 0x00] [Length LSB: 0x03]
        cat205_packet = bytearray([
            0xCD,  # Category 205
            0x00,  # Length MSB
            0x03,  # Length LSB = 3 bytes (header only)
        ])

        result = asterix.parse(cat205_packet)
        ...
        if len(result) > 0:
            assert result[0]['category'] == 205, f"Should identify as CAT205"

    def test_parse_cat205_with_data_items(self):
        """
        Test parsing CAT205 with data items

        Requirement: REQ-LLR-205-010
        Verification: TC-PY-CAT205-002
        """
        # Create CAT205 packet with I205/010 (Data Source Identifier)
        # Structure: [Category: 0xCD] [Length MSB: 0x00] [Length LSB: 0x08] [FSPEC: 0x80] [I205/010: 2 bytes]
        cat205_packet = bytearray([
            0xCD,  # Category 205
            0x00,  # Length MSB
    ...
    def test_cat205_api_usage(self):
        """
        Test Python API usage for CAT205

        Requirement: REQ-HLR-001
        Verification: TC-PY-CAT205-004
        """
        ...
        result_verbose = asterix.parse(cat205_packet, verbose=True)
        result_quiet = asterix.parse(cat205_packet, verbose=False)
