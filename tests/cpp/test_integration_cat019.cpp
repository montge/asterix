/**
 * Integration test for CAT019 parsing
 *
 * This test verifies the complete ASTERIX parsing pipeline for Category 019
 ...
 * - Additional requirements from Low_Level_Requirements_CAT019.md
 *
 * Test Cases:
 * - TC-INT-CAT019-001: Load CAT019 XML configuration
 * - TC-INT-CAT019-002: Parse CAT019 binary data
 * - TC-INT-CAT019-003: Verify parsed data structure
 * - TC-INT-CAT019-004: Reject invalid data
 * - TC-INT-CAT019-005: Verify text output generation
 */
...
class CAT019IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(CAT019IntegrationTest, LoadXMLConfiguration) {
    const char* configFile = "../asterix/config/asterix_cat019_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile)) << "Failed to load CAT019 XML configuration";
    ASSERT_TRUE(pDefinition->CategoryDefined(19)) << "CAT019 category not defined";
    Category* cat = pDefinition->getCategory(19);
    ASSERT_NE(cat, nullptr) << "CAT019 category is NULL";
    EXPECT_EQ(cat->m_id, 19) << "Category ID should be 19";
}

TEST_F(CAT019IntegrationTest, ParseCAT019BinaryData) {
    const char* configFile = "../asterix/config/asterix_cat019_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    unsigned char buffer[256];
    buffer[0] = 0x13;  // Category 19
...
TEST_F(CAT019IntegrationTest, VerifyParsedDataStructure) {
    const char* configFile = "../asterix/config/asterix_cat019_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT019IntegrationTest, RejectInvalidData) {
    const char* configFile = "../asterix/config/asterix_cat019_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT019IntegrationTest, VerifyTextOutput) {
    const char* configFile = "../asterix/config/asterix_cat019_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
