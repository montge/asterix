/**
 * Integration test for CAT32 parsing
 *
 * This test verifies the complete ASTERIX parsing pipeline for Category 32
 ...
 * - Additional requirements from Low_Level_Requirements_CAT32.md
 *
 * Test Cases:
 * - TC-INT-CAT32-001: Load CAT32 XML configuration
 * - TC-INT-CAT32-002: Parse CAT32 binary data
 * - TC-INT-CAT32-003: Verify parsed data structure
 * - TC-INT-CAT32-004: Reject invalid data
 * - TC-INT-CAT32-005: Verify text output generation
 */
...
class CAT32IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(CAT32IntegrationTest, LoadXMLConfiguration) {
    const char* configFile = "../asterix/config/asterix_cat032_7_0.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile)) << "Failed to load CAT32 XML configuration";
    ASSERT_TRUE(pDefinition->CategoryDefined(32)) << "CAT32 category not defined";
    Category* cat = pDefinition->getCategory(32);
    ASSERT_NE(cat, nullptr) << "CAT32 category is NULL";
    EXPECT_EQ(cat->m_id, 32) << "Category ID should be 32";
}

TEST_F(CAT32IntegrationTest, ParseCAT32BinaryData) {
    const char* configFile = "../asterix/config/asterix_cat032_7_0.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    unsigned char buffer[256];
    buffer[0] = 0x20;  // Category 32
...
TEST_F(CAT32IntegrationTest, VerifyParsedDataStructure) {
    const char* configFile = "../asterix/config/asterix_cat032_7_0.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT32IntegrationTest, RejectInvalidData) {
    const char* configFile = "../asterix/config/asterix_cat032_7_0.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT32IntegrationTest, VerifyTextOutput) {
    const char* configFile = "../asterix/config/asterix_cat032_7_0.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
