/**
 * Integration test for CAT63 parsing
 *
 * This test verifies the complete ASTERIX parsing pipeline for Category 63
 ...
 * - Additional requirements from Low_Level_Requirements_CAT63.md
 *
 * Test Cases:
 * - TC-INT-CAT63-001: Load CAT63 XML configuration
 * - TC-INT-CAT63-002: Parse CAT63 binary data
 * - TC-INT-CAT63-003: Verify parsed data structure
 * - TC-INT-CAT63-004: Reject invalid data
 * - TC-INT-CAT63-005: Verify text output generation
 */
...
class CAT63IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(CAT63IntegrationTest, LoadXMLConfiguration) {
    const char* configFile = "../asterix/config/asterix_cat063_1_6.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile)) << "Failed to load CAT63 XML configuration";
    ASSERT_TRUE(pDefinition->CategoryDefined(63)) << "CAT63 category not defined";
    Category* cat = pDefinition->getCategory(63);
    ASSERT_NE(cat, nullptr) << "CAT63 category is NULL";
    EXPECT_EQ(cat->m_id, 63) << "Category ID should be 63";
}

TEST_F(CAT63IntegrationTest, ParseCAT63BinaryData) {
    const char* configFile = "../asterix/config/asterix_cat063_1_6.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    unsigned char buffer[256];
    buffer[0] = 0x3F;  // Category 63
...
TEST_F(CAT63IntegrationTest, VerifyParsedDataStructure) {
    const char* configFile = "../asterix/config/asterix_cat063_1_6.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT63IntegrationTest, RejectInvalidData) {
    const char* configFile = "../asterix/config/asterix_cat063_1_6.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT63IntegrationTest, VerifyTextOutput) {
    const char* configFile = "../asterix/config/asterix_cat063_1_6.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
