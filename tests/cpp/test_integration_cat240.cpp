 ...
 *
 * This test verifies the complete ASTERIX parsing pipeline for Category 240
 ...
 * - Additional requirements from Low_Level_Requirements_CAT240.md
 *
 * Test Cases:
 * - TC-INT-CAT240-001: Load CAT240 XML configuration
 * - TC-INT-CAT240-002: Parse CAT240 binary data
 * - TC-INT-CAT240-003: Verify parsed data structure
 * - TC-INT-CAT240-004: Reject invalid data
 * - TC-INT-CAT240-005: Verify text output generation
 */
...
class CAT240IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(CAT240IntegrationTest, LoadXMLConfiguration) {
    const char* configFile = "../asterix/config/asterix_cat240_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile)) << "Failed to load CAT240 XML configuration";
    ASSERT_TRUE(pDefinition->CategoryDefined(240)) << "CAT240 category not defined";
    Category* cat = pDefinition->getCategory(240);
    ASSERT_NE(cat, nullptr) << "CAT240 category is NULL";
    EXPECT_EQ(cat->m_id, 240) << "Category ID should be 240";
}

TEST_F(CAT240IntegrationTest, ParseCAT240BinaryData) {
    const char* configFile = "../asterix/config/asterix_cat240_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    unsigned char buffer[256];
    buffer[0] = 0xF0;  // Category 240
...
TEST_F(CAT240IntegrationTest, VerifyParsedDataStructure) {
    const char* configFile = "../asterix/config/asterix_cat240_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT240IntegrationTest, RejectInvalidData) {
    const char* configFile = "../asterix/config/asterix_cat240_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
...
TEST_F(CAT240IntegrationTest, VerifyTextOutput) {
    const char* configFile = "../asterix/config/asterix_cat240_1_3.xml";
    ASSERT_TRUE(LoadXMLConfig(configFile));
    pParser = new InputParser(pDefinition);
    pDefinition = nullptr;
